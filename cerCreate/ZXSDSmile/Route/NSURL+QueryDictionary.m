...
//      }else{
//          key = [components[0] stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
//      }
  ...
  for (NSString *key in keys) {
    ...
    // beware of empty or null
      if ([rawValue isKindOfClass:[NSString class]] || [rawValue isKindOfClass:[NSNumber class]]) {
          value = uq_URLEscape([rawValue description]);
      }
...
static inline NSString *uq_URLEscape(NSString *string) {
    ...
    return [string stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet characterSetWithCharactersInString:uq_URLReservedChars]];
