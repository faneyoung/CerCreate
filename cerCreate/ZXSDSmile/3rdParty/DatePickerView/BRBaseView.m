...
- (UIView *)maskView {
    if (!_maskView) {
        _maskView = [[UIView alloc] initWithFrame:BR_SCREEN_BOUNDS];
...
- (UIView *)alertView {
    if (!_alertView) {
        ...
        _alertView = [[UIView alloc]initWithFrame:CGRectMake(0, BR_SCREEN_HEIGHT - height, BR_SCREEN_WIDTH, height)];
...
- (UIView *)titleBarView {
    if (!_titleBarView) {
        _titleBarView =[[UIView alloc]initWithFrame:CGRectMake(0, 0, BR_SCREEN_WIDTH, self.pickerStyle.titleBarHeight)];
...
- (void)addPickerToView:(UIView *)view {
    if (view) {
        ...
        rect.origin.y = BR_SCREEN_HEIGHT;
