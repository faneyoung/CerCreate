//
//  ZXSDUploadWechatDetailsController.m
//  ZXSDSmile
//
...
static const NSString *GET_WECHAT_DETAILS_URL = @"/rest/certificate/getEmail?certType=cert_weixin_flow";
static const NSString *UPLOAD_WECHAT_DETAILS_URL = @"/rest/certificate/submit";

@interface ZXSDUploadWechatDetailsController ()<UITextFieldDelegate>{
    UILabel *_wechatDetailsLabel;
...
@implementation ZXSDUploadWechatDetailsController
...
- (void)prepareDataConfigure {
    ...
    [manager GET:[NSString stringWithFormat:@"%@%@",MAIN_URL,GET_WECHAT_DETAILS_URL] parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ZGLog(@"获取微信账单邮箱信息接口成功返回数据---%@",responseObject);
        ...
        self->_wechatDetails = model.title;
        self->_email = model.recipient;
        self->_typeName = model.flowFullName;
...
- (void)addUserInterfaceConfigure {
    ...
    _wechatDetailsLabel = [UILabel labelWithText:@"" textColor:UICOLOR_FROM_HEX(0xC8A028) font:FONT_PINGFANG_X(12)];
    _wechatDetailsLabel.frame = CGRectMake(0, 0, SCREEN_WIDTH(), 40);
    _wechatDetailsLabel.textAlignment = NSTextAlignmentCenter;
    _wechatDetailsLabel.backgroundColor = UIColorFromHex(0xFFF4D4);
    [_scrollView addSubview:_wechatDetailsLabel];
    
    UIImageView *bankImageView = [[UIImageView alloc] initWithImage:UIIMAGE_FROM_NAME(@"icon_upload_detail")];
    bankImageView.frame = CGRectMake((SCREEN_WIDTH() - 219)/2, CGRectGetMaxY(_wechatDetailsLabel.frame) + 40, 219, 166);
    ...
    _contentLabel.text = @"找到微信账单明细，并发送到邮箱";
    ...
    UILabel *wechatGuideLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, SCREEN_HEIGHT() - NAVIBAR_HEIGHT() - 24 - 20 - safaAreaBottom, SCREEN_WIDTH() - 40, 20)];
    wechatGuideLabel.text = @"微信账单明细指引";
    wechatGuideLabel.textColor = UICOLOR_FROM_HEX(0x5886E2);
    wechatGuideLabel.font = FONT_PINGFANG_X(14);
    wechatGuideLabel.textAlignment = NSTextAlignmentCenter;
    wechatGuideLabel.userInteractionEnabled = YES;
    [self.view addSubview:wechatGuideLabel];
    
    UITapGestureRecognizer *wechatGuideTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(jumpToWechatkGuideH5)];
    [wechatGuideLabel addGestureRecognizer:wechatGuideTap];
}

- (void)updateUserInterfaceConfigure {
    _wechatDetailsLabel.text = _wechatDetails;
...
//    [_scrollView addSubview:copyButton];
    ...
    if (_isShowZipTextField) {
        if (_isNecessaryInput) {
            ...
            [rightButton setTitleColor:UICOLOR_FROM_HEX(0x00B050) forState:UIControlStateNormal];
            rightButton.titleLabel.font = [UIFont fontWithName:@"PingFangSC-Regular" size:14.0];
...
- (void)submitButtonClicked {
    ...
    NSString *certificateType = @"cert_weixin_flow";
    if (_isShowZipTextField ) {
        if (_isNecessaryInput) {
            //必填解压码
            parameters = @{@"bankName":_typeName,@"certificateType":certificateType,@"zipCode":_zipTextField.text};
        } else {
            //不确定是否要填,用户未点无密码的时候
            if (_zipTextField.text.length > 0) {
                parameters = @{@"bankName":_typeName,@"certificateType":certificateType,@"zipCode":_zipTextField.text};
            } else {
                parameters = @{@"bankName":_typeName,@"certificateType":certificateType
                    ,@"zipCode":@""};
            }
        }
    } else {
    ...
    [self showLoadingProgressHUDWithText:@"正在加载..."];
    AFHTTPSessionManager *manager = [ZXSDPublicClassMethod getAFSessionManagerWithRequestType:AFSerializerTypeJson andResponseType:AFSerializerTypeJson];
    [manager POST:[NSString stringWithFormat:@"%@%@",MAIN_URL,UPLOAD_WECHAT_DETAILS_URL] parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ZGLog(@"提交微信账单明细信息接口成功返回数据---%@",responseObject);
        [self dismissLoadingProgressHUD];
